...
consultas.forEach((consulta) =>{
    ...
    if(novoCard.querySelector(".especialidade").innerHTML == "Clinico Geral"){
      novoCard.querySelector(".especialidade").innerHTML = "Clinico"
    }
});
...
btnFiltrar.addEventListener("click", (event) => {
    ...
    Array.from(main.children).forEach(box => {

        const campoCat = box.querySelector(".especialidade");

        const catBox = campoCat.textContent;

        if (categoriaSelecionada === "0") {
            box.style.display = "block";
        } else if (catBox === categoriaSelecionada) {
            box.style.display = "block";
        } else {
